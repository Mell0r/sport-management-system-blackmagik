...
class Application(
    val teamName: String,
    val applicantsList: List<List<String>>
) : CsvDumpable {
    companion object : CreatableFromFileContent<Application> {
        override fun readFromFileContent(fileContent: FileContent): Application {
            ...
            val applicantsList =
                application.drop(1).filterIndexed { applicantInd, applicant ->
                    if (applicant[3].toIntOrNull() == null)
                        Logger.warn { "Applicant number $applicantInd has incorrect birth year, so he/she is not allowed to competition." }
                    applicant[3].toIntOrNull() != null
                }.map { listOf(it[3], it[2], it[1], it[0], teamName, it[4]) }
            return Application(teamName, applicantsList)
        }
    }

    override fun dumpToCsv() =
        listOf("$teamName,,,,") + applicantsList.map { it.joinToString(",") }
}